...
public class VideoView extends TextureView implements TextureView.SurfaceTextureListener {
    ...
    protected void initVideoView() {
        ...
        setDrawingCacheBackgroundColor(getResources().getColor(R.color.colorb));
    }
...
//		com.our.doing.util.Utils.LogE(e.getMessage());
        Utils.LogE("tryAgain");
    ...
    public void openVideo(Uri uri) {
        ...
        Utils.LogE("[LocalVideoView]openVideo...");
        ...
        try {
            if (mMediaPlayer == null) {
                Utils.LogE("[LocalVideoView]openVideo2...");
...
//                mMediaPlayer.setAuxEffectSendLevel()
            } else {
                mMediaPlayer.reset();
            }
...
//			mMediaPlayer.setAudioStreamType();
            ...
            exception = ex;
            Utils.LogE("IOException=" + ex.getMessage());
        } catch (IllegalArgumentException ex) {
            exception = ex;
            Utils.LogE("IllegalArgumentException=" + ex.getMessage());
        } catch (Exception ex) {
            exception = ex;
            Utils.LogE("Exception=" + ex.getMessage());
        }
        if (exception != null) {
            mCurrentState = STATE_ERROR;
        ...
        public void onPrepared(MediaPlayer mp) {
            ...
            if (mCurrentState == STATE_PREPARING) {
                ...
                try {
                    ...
                    if (mOnPlayStateListener != null)
                        mOnPlayStateListener.onDuration(mDuration);
                } catch (IllegalStateException e) {
                    Utils.LogE(e.getMessage());
                }
                ...
                if (mVideoWidth > 0 && mVideoHeight > 0) {
                    if ((mVideoWidth / mVideoHeight) > (displayWidth / displayHeight)) {
                        viewWidth = displayWidth;
                        viewHeight = displayWidth / mVideoWidth * mVideoHeight;
                    } else {
                        viewWidth = displayHeight / mVideoHeight * mVideoWidth;
...
//                private SurfaceTexture mSurfaceHolder = null;
    ...
    private MediaPlayer.OnErrorListener mErrorListener = new MediaPlayer.OnErrorListener() {
        ...
        public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
            Utils.LogE("[VideoView]Error:" + framework_err + "," + impl_err);
            mCurrentState = STATE_ERROR;
