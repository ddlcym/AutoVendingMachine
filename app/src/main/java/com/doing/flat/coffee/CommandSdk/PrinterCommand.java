...
public class PrinterCommand {
  ...
  public static byte[] POS_Set_CodePage(int page){
	...
	public static byte[] POS_Print_Text(int codepage, String pszString, String encoding ) {
		...
		if(codepage == 0){
			byte[] data = Other.byteArraysToBytes(new byte[][]{
					Command.ESC_t, Command.FS_and, pbString});

			return data;
		}else{
			byte[] data = Other.byteArraysToBytes(new byte[][]{
					Command.ESC_t, Command.FS_dot, pbString});
