...
import com.doing.flat.coffee.download.utils.ConfigUtils;
import com.doing.flat.coffee.download.utils.MyIntents;
import com.doing.flat.coffee.download.utils.NetworkUtils;
import com.doing.flat.coffee.download.utils.StorageUtils;
...
public class DownloadManager extends Thread {
    ...
    public void setNoti(boolean isNoti) {
        this.isNoti = isNoti;
    }
...
//            Utils.openFile(mContext, file);//这里是安装
        ...
        for (int i = 0; i < mDownloadingTasks.size(); i++) {
            ...
            if (task != null && task.getUrl().equals(url)) {
                ...
                if (isNoti) {
                    ...
                    mBuilder.setContentText("已暂停，点击继续下载").setProgress(100,
                            task.getOldProcess(), false);
    ...
    public synchronized void completeTask(DownloadTask task) {
        ...
        for (int i = 0; i < mDownloadingTasks.size(); i++) {
            if (task.getUrl().endsWith(mDownloadingTasks.get(i).getUrl())) {
                ...
                mDownloadingTasks.remove(task);
                if (isNoti) {
                    Intent intent = StorageUtils.getInstallAPKIntent(mContext,
                            task.getUrl());
